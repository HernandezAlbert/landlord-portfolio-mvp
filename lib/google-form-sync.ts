...
function eligibleForAutomaticImport(property: {
  tenancies: { id: string; isActive?: boolean }[];
  googleFormImportEnabled: boolean;
  googleSheetId: string | null;
  googleSheetTabName: string | null;
}) {
  const hasActiveTenancy = property.tenancies.some(
    (tenancy) => tenancy.isActive === true,
  );

  return (
    property.googleFormImportEnabled &&
    !!property.googleSheetId &&
    !!property.googleSheetTabName &&
    !hasActiveTenancy
  );
}
...
function buildApplicantNotificationEmail(args: {
  ...
  return {
    ...
    html: `
      ...
      <p><strong>Applicant:</strong> ${args.fullName}</p>
      <p><strong>Email:</strong> ${args.email ?? "—"}</p>
      <p><strong>Phone:</strong> ${args.phone ?? "—"}</p>
      <p><strong>Pre-screen decision:</strong> ${args.decision}</p>
      <p><strong>Screening status:</strong> ${args.screeningStatus}</p>
      <p><strong>Summary:</strong> ${args.screeningSummary}</p>
    `,
  };
}

export async function syncApplicantsForProperty(
  propertyId: string,
  ...
  const property = await prisma.property.findUnique({
    ...
    include: {
      tenancies: {
        ...
        select: { id: true, rentMonthly: true, isActive: true },
  ...
  if (!eligibleForAutomaticImport(property)) {
    return {
      ...
      skipped: 0,
      checkedRows: 0,
      skippedReason: property.tenancies.some((tenancy) => tenancy.isActive === true)
        ? "Property has an active tenancy."
        : "Google Form import is not fully configured.",
    } satisfies PropertySyncOutcome;
  }

  try {
    const table = await readGoogleSheetValues(
      property.googleSheetId!,
      `${property.googleSheetTabName}`,
    );
    ...
    const rentMonthly =
      property.advertisedRentMonthly ?? property.tenancies[0]?.rentMonthly ?? null;

    let imported = 0;
    let updated = 0;
    let skipped = 0;

    const lastImportedRow = Math.max(
      property.googleLastImportedRow ?? 1,
      parsedRows.length + 1,
    );
    ...
    await prisma.property.update({
      ...
      data: {
        ...
        googleLastImportedAt:
          imported > 0 || updated > 0 ? new Date() : property.googleLastImportedAt,
    ...
    return {
      ...
      updated,
      skipped,
      checkedRows: candidateRows.length,
    } satisfies PropertySyncOutcome;
  } catch (error) {
    ...
    await prisma.property.update({
      ...
      data: {
        googleLastCheckedAt: new Date(),
        googleSyncError: message.slice(0, 5000),
      },
    });
...
export async function syncApplicantsForEligibleProperties(options?: {
  sendEmails?: boolean;
}) {
