    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        <StatCard
          ...
          colour={totalArrears > 0 ? "text-red-700" : "text-green-700"}
          href="/actions"
        />
        <StatCard
          title="Red Items"
          value={redCount}
          colour={redCount > 0 ? "text-red-700" : "text-green-700"}
          href="/actions"
        />
        <StatCard
          title="Section 8 Eligible"
          value={s8Count}
          colour={s8Count > 0 ? "text-amber-700" : "text-green-700"}
          href="/actions"
        />
        <StatCard
          title="RTR due ≤60d"
          value={rightToRentDueCount}
          colour={rightToRentDueCount > 0 ? "text-amber-700" : "text-green-700"}
          href="/actions"
        />
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        <StatCard
          ...
          value={money(finance.cashReceivedThisMonth)}
          colour={finance.cashReceivedThisMonth > 0 ? "text-green-700" : "text-slate-900"}
          href="/finance"
        />
        <StatCard
          title="Outstanding This Month"
          value={money(finance.outstandingThisMonth)}
          colour={finance.outstandingThisMonth > 0 ? "text-red-700" : "text-green-700"}
          href="/finance"
        />
        <StatCard title="Expenses This Month" value={money(finance.expensesThisMonth)} colour="text-slate-900" href="/expenses" />
        <StatCard
          title="Net Cashflow"
          value={money(finance.estimatedNetCashflow)}
          colour={finance.estimatedNetCashflow >= 0 ? "text-green-700" : "text-red-700"}
          href="/finance"
        />
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {actions.slice(0, 10).map((a, idx) => {
                  const href = a.tenantId
                    ? `/tenants/${a.tenantId}/edit`
                    : a.tenancyId
                    ? `/tenancies/${a.tenancyId}`
                    : a.propertyId
        ...
        <div className="bg-white border rounded-xl p-4">
          ...
          <div className="space-y-3">
              ...
              return (
                <div key={payment.id} className="rounded-lg border border-slate-200 p-3">
                  ...
                  <div className="mt-1 text-sm text-slate-700">
                    Due {payment.dueDate.toISOString().slice(0, 10)} · Outstanding {money(outstanding)}
                  </div>
            ...
            {!finance.overduePayments.length && (
              <p className="text-sm text-slate-500">No overdue payments right now.</p>
            )}
          </div>
        </div>
      </div>
