...
async function getApplicantDocumentCount(applicantId: string) {
  const folder = path.join(process.cwd(), "public", "uploads", "applicants", applicantId);
  try {
...
export default async function ApplicantsPage({
  ...
  const importError = typeof qs.importError === "string" ? decodeURIComponent(qs.importError) : "";
  const importStatus = typeof qs.imported === "string" ? decodeURIComponent(qs.imported) : "";
  const cleanupStatus = typeof qs.cleanup === "string" ? decodeURIComponent(qs.cleanup) : "";
  const success = typeof qs.success === "string" ? decodeURIComponent(qs.success) : "";
  const archivedMode = typeof qs.archived === "string" && qs.archived === "1";

  const [applicants, properties] = await Promise.all([
    prisma.applicant.findMany({
      where: archivedMode ? { deletedAt: { not: null } } : { deletedAt: null },
  ...
  const [activeCount, archivedCount] = await Promise.all([
    ...
    prisma.applicant.count({ where: { deletedAt: { not: null } } }),
  ]);

  const propertyRentMap = new Map(properties.map((property) => [property.id, property.advertisedRentMonthly ?? null]));
  ...
  async function createApplicant(formData: FormData) {
    ...
    const employmentStatus = String(formData.get("employmentStatus") ?? "").trim() || null;
    ...
    const savingsBufferMonths = Number(formData.get("savingsBufferMonths") ?? 0) || null;
    ...
    const applicant = await prisma.applicant.create({
      data: {
        ...
        propertyId,
        employmentStatus,
        monthlyIncome: monthlyIncomePounds ? Math.round(monthlyIncomePounds * 100) : null,
  ...
  async function importGoogleFormResponses(formData: FormData) {
    ...
    try {
      ...
      if (!parsedRows.length) {
        throw new Error("No valid applicant rows were found. Check the Google Sheet tab and column headings.");
      }
      ...
      const propertyForImport = propertyId
        ...
        : null;
      const rentMonthly = propertyForImport?.advertisedRentMonthly ?? propertyForImport?.tenancies[0]?.rentMonthly ?? null;
      let importedCount = 0;
      let updatedCount = 0;
      ...
      for (const row of parsedRows) {
        const result = await upsertImportedApplicant({
          ...
          guarantorMinMultiplier: propertyForImport?.screeningGuarantorMinMultiplier ?? 2.0,
      ...
      redirectUrl = `/applicants?imported=${encodeURIComponent(`Imported ${importedCount} new row(s). Updated ${updatedCount} existing row(s).`)}`;
    } catch (error) {
  ...
  async function dedupeImportedApplicants() {
    ...
    revalidatePath("/applicants");
    redirect(`/applicants?cleanup=${encodeURIComponent(`Deduplicated ${duplicateIds.size} imported applicant(s).`)}`);
  }
  ...
  async function restoreApplicant(formData: FormData) {
    ...
    revalidatePath("/applicants");
    redirect(`/applicants?archived=1&success=${encodeURIComponent("Applicant restored.")}`);
  }
  ...
  async function deleteArchivedApplicant(formData: FormData) {
    ...
    if (!existing?.deletedAt) {
      redirect(`/applicants?archived=1&importError=${encodeURIComponent("Only archived applicants can be deleted permanently.")}`);
    }
  ...
  const enriched = await Promise.all(
    applicants.map(async (a) => {
      const result = computeReferencingScore({
        monthlyIncome: getIncomeBreakdownFromRawPayload(a.importRawPayload).totalMonthlyPence ?? a.monthlyIncome,
      ...
      const effectiveDecision = getEffectiveDecision({ computedDecision, manualDecision });
      const effectiveStatus = getApplicantStatusFromDecision({ decision: effectiveDecision, currentStatus: a.status });
  ...
  const stats = {
    ...
    inPipeline: enriched.filter((a) => ["APPLIED", "REFERENCING", "MORE_INFO_REQUESTED"].includes(a.effectiveStatus)).length,
    approved: enriched.filter((a) => a.effectiveStatus === "APPROVED").length,
    averageScore: enriched.length ? Math.round(enriched.reduce((sum, a) => sum + a.displayScore, 0) / enriched.length) : 0,
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Applicants &amp; Referencing</h1>
          <p className="mt-1 text-sm text-slate-500">
            Track applicants, manual decisions, notes, archive state, document uploads, referencing progress and conversion into live tenants.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link
            href="/applicants"
            className={`rounded-lg border px-4 py-2 text-sm font-medium ${!archivedMode ? "border-slate-900 bg-slate-900 text-white" : "border-slate-300 bg-white text-slate-700 hover:bg-slate-50"}`}
          >
      ...
      {importError ? <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{importError}</div> : null}
      {importStatus ? <div className="rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">{importStatus}</div> : null}
      {cleanupStatus ? <div className="rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700">{cleanupStatus}</div> : null}
      ...
      {!archivedMode ? (
        <>
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                <h2 className="text-lg font-semibold text-slate-900">Applicant cleanup tools</h2>
                <p className="mt-1 text-sm text-slate-500">
                  Use this after testing imports to remove duplicate imported applicants while keeping the earliest record.
                </p>
              </div>
          ...
          <section className="grid gap-4 lg:grid-cols-2">
            <details className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-3">
                <div>
                  <h2 className="text-lg font-semibold text-slate-900">Import Google Form responses</h2>
                  <p className="mt-1 text-sm text-slate-500">Keep this tucked away unless you are importing a new batch.</p>
                </div>
                <span className="details-chip">Open import</span>
              </summary>
              <p className="mt-4 text-sm text-slate-500">
                Paste either the Google Sheet share link or the CSV export link. The importer converts it to CSV, updates matching rows,
                ignores blank rows, and maps your current RM9 pre-screening headers automatically.
              </p>
              <form action={importGoogleFormResponses} className="mt-4 grid gap-4 md:grid-cols-[2fr_1fr_auto] md:items-end">
                ...
                <label className="grid gap-1 text-sm">
                  Assign to property
                  <select name="importPropertyId" className="rounded-lg border px-3 py-2" defaultValue="">
            ...
            <details className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-3">
                <div>
                  <h2 className="text-lg font-semibold text-slate-900">Add applicant</h2>
                  <p className="mt-1 text-sm text-slate-500">Create a manual applicant only when needed.</p>
                </div>
              ...
              <form action={createApplicant} className="mt-4 grid gap-4 md:grid-cols-2">
                <label className="grid gap-1 text-sm">
                  Full name
                  <input name="fullName" className="rounded-lg border px-3 py-2" required />
                </label>
                <label className="grid gap-1 text-sm">
                  Property
                  <select name="propertyId" className="rounded-lg border px-3 py-2" defaultValue="">
                    <option value="">Unassigned</option>
                    {properties.map((p) => (
                      <option key={p.id} value={p.id}>
                        {p.name} — {p.postcode}
                ...
                <label className="grid gap-1 text-sm">
                  Employment status
                  <input name="employmentStatus" placeholder="Permanent / Contract / Self-employed" className="rounded-lg border px-3 py-2" />
                </label>
                <label className="grid gap-1 text-sm">
                  Monthly income (£)
                  <input type="number" step="0.01" min="0" name="monthlyIncome" className="rounded-lg border px-3 py-2" />
                </label>
                <label className="grid gap-1 text-sm">
                  Requested move-in date
                  <input type="date" name="requestedMoveIn" className="rounded-lg border px-3 py-2" />
                </label>
                <div className="grid grid-cols-3 gap-3">
                  <label className="grid gap-1 text-sm">
                    Adults
                    <input type="number" min="1" name="adults" defaultValue={1} className="rounded-lg border px-3 py-2" />
                  </label>
                  <label className="grid gap-1 text-sm">
                    Children
                    <input type="number" min="0" name="children" defaultValue={0} className="rounded-lg border px-3 py-2" />
                  </label>
                  <label className="grid gap-1 text-sm">
                    Savings buffer (months)
                    <input type="number" min="0" name="savingsBufferMonths" defaultValue={0} className="rounded-lg border px-3 py-2" />
                  </label>
                </div>
                <div className="grid gap-2">
                  <label className="grid gap-1 text-sm">
                    Pets?
                    <select name="hasPets" defaultValue="false" className="rounded-lg border px-3 py-2">
                  ...
                  <label className="grid gap-1 text-sm">
                    Pet details
                    <input name="petDetails" placeholder="Type / breed / age" className="rounded-lg border px-3 py-2" />
                  </label>
                </div>
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm text-sm text-slate-600">
          Archived applicants are hidden from the live workflow. Restore them to bring them back into the active list, or delete them permanently here.
        </section>
      )}
            ...
            {enriched.map((a) => (
              <tr key={a.id} className="border-t border-slate-200 align-top">
                <td className="px-4 py-3">
                  <Link href={`/applicants/${a.id}`} className="font-medium text-blue-600 hover:underline">
                    {a.fullName}
                  </Link>
                  <div className="mt-1 flex flex-wrap gap-2">
                    <Pill className={getApplicantStatusTone(a.effectiveStatus)}>{formatApplicantStatus(a.effectiveStatus)}</Pill>
                    {a.manualDecision ? <Pill className="border-violet-200 bg-violet-50 text-violet-700">Manual override</Pill> : null}
                    <Pill className="border-slate-200 bg-slate-50 text-slate-700">{a.documentCount} docs</Pill>
                    {a.deletedAt ? <Pill className="border-slate-300 bg-slate-100 text-slate-700">Archived</Pill> : null}
                  </div>
                  <div className="mt-2 text-xs text-slate-500">{a.email ?? a.phone ?? "No contact details"}</div>
                </td>
                <td className="px-4 py-3">{a.property?.name ?? "Unassigned"}</td>
                <td className="px-4 py-3">
                  <div className="font-medium text-slate-900">{formatMoney(a.effectiveIncome)}</div>
                  {a.incomeBreakdown.additionalMonthlyPence ? (
                    <div className="mt-1 space-y-1 text-xs text-slate-500">
                      <div>Base: {formatMoney(a.incomeBreakdown.baseMonthlyPence)}</div>
                      <div>Additional: {formatMoney(a.incomeBreakdown.additionalMonthlyPence)}</div>
                    </div>
                  ) : null}
                </td>
                <td className="px-4 py-3">
                  <Pill className={getScreeningTone(a.screeningStatus)}>{formatScreeningStatus(a.screeningStatus)}</Pill>
                  <div className="mt-2 text-xs text-slate-500">{a.screeningSummary ?? "Initial screen passed."}</div>
                </td>
                <td className="px-4 py-3">
                  <div className="text-sm font-medium text-slate-900">{a.completion.percent}% complete</div>
                  <div className="mt-1 h-2 w-32 overflow-hidden rounded-full bg-slate-100">
                    <div className="h-full rounded-full bg-blue-600" style={{ width: `${a.completion.percent}%` }} />
                  </div>
                  <div className="mt-1 text-xs text-slate-500">{a.missingItems} risk item(s)</div>
                </td>
                <td className="px-4 py-3">
                  <Pill className={getDecisionTone(a.displayDecision)}>{a.displayDecision}</Pill>
                  <div className="mt-2 text-xs text-slate-500">Score: {a.displayScore}</div>
                </td>
                <td className="px-4 py-3 text-xs text-slate-500">{a.importSource ?? "Manual"}</td>
                <td className="px-4 py-3">
                  <div className="flex flex-col gap-2">
                    <Link href={`/applicants/${a.id}`} className="rounded-lg border border-slate-300 px-3 py-2 text-center text-xs font-medium text-slate-700 hover:bg-slate-50">
                      Open
                    </Link>
                    {!archivedMode ? (
                      <form action={archiveApplicant}>
                        <input type="hidden" name="id" value={a.id} />
                        <button type="submit" className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50">
                          Archive
                        </button>
                      </form>
                    ) : (
                      <>
                        ...
                        <form action={deleteArchivedApplicant}>
                          <input type="hidden" name="id" value={a.id} />
                          <button type="submit" className="w-full rounded-lg bg-red-600 px-3 py-2 text-xs font-medium text-white hover:bg-red-700">
                            Delete
                          </button>
                        </form>
                      </>
