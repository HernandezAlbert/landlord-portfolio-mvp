...
export default async function PropertyPage({
  ...
  const googleSync =
    typeof qs.googleSync === "string" ? decodeURIComponent(qs.googleSync) : "";
  const googleError =
    typeof qs.googleError === "string" ? decodeURIComponent(qs.googleError) : "";
  ...
  async function saveGoogleImportSettings(formData: FormData) {
    ...
    const enabled =
      String(formData.get("googleFormImportEnabled") ?? "false") === "true";
    const googleSheetId =
      extractSpreadsheetId(String(formData.get("googleSheetId") ?? "").trim()) ||
      null;
    const googleSheetTabName =
      String(formData.get("googleSheetTabName") ?? "").trim() || null;
    const screeningPassMultiplier = Math.max(
      1,
      Number(formData.get("screeningPassMultiplier") ?? 3) || 3,
    );
    const advertisedRentMonthlyPounds = Number(
      formData.get("advertisedRentMonthly") ?? 0,
    );
    const screeningGuarantorMinMultiplierRaw =
      Number(formData.get("screeningGuarantorMinMultiplier") ?? 2.0) || 2.0;
    const screeningGuarantorMinMultiplier = Math.max(
      1,
      Math.min(screeningPassMultiplier, screeningGuarantorMinMultiplierRaw),
    );
    const resetLastRow =
      String(formData.get("resetLastRow") ?? "false") === "true";
    ...
    await prisma.property.updateMany({
      ...
      data: {
        ...
        advertisedRentMonthly: advertisedRentMonthlyPounds
          ...
          : null,
        googleLastImportedRow: resetLastRow
          ? null
          : (property?.googleLastImportedRow ?? null),
  ...
  async function runApplicantSyncNow() {
    ...
    if (result.skippedReason) {
      redirect(
        `/properties/${id}?googleError=${encodeURIComponent(
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        ...
        <div className="flex gap-2">
          ...
          <Link href="/properties" className="px-3 py-2 border rounded-lg">
            Back
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="border rounded-lg p-4 bg-white">
          ...
          <div className="text-xl font-bold">
            {fmt(property.mortgage?.productEndDate ?? null)}
          </div>
        </div>
      </div>
      ...
      <section className="rounded-xl border bg-white p-4 space-y-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Vercel-ready applicant import for this property. Only properties
              without an active tenancy are checked automatically.
            </p>
          </div>
        ...
        <form action={saveGoogleImportSettings} className="grid gap-4 lg:grid-cols-2">
          <label className="grid gap-1 text-sm lg:col-span-2">
            <span className="font-medium text-slate-800">
              Enable automatic Google Form import
            </span>
            <select
          ...
          <label className="grid gap-1 text-sm">
            ...
            <span className="text-xs text-slate-500">
              Share the response sheet with your Google service account email
          ...
          <label className="grid gap-1 text-sm">
            ...
            />
            <span className="text-xs text-slate-500">
              Usually <code>Form Responses 1</code>.
            </span>
          </label>
          ...
          <label className="grid gap-1 text-sm">
            ...
            <input
              ...
              step="0.01"
              defaultValue={
                property.advertisedRentMonthly
            ...
            />
            <span className="text-xs text-slate-500">
              Used for applicant screening before a tenancy exists. If blank, the
              app falls back to an active tenancy rent when available.
            </span>
          </label>

          <label className="grid gap-1 text-sm">
            <span className="font-medium text-slate-800">
              Pass affordability multiplier
            </span>
            <input
            ...
            />
            <span className="text-xs text-slate-500">
              Applicants at or above this multiple of monthly rent are screened
              green.
            </span>
          </label>
          ...
          <label className="grid gap-1 text-sm">
            ...
            />
            <span className="text-xs text-slate-500">
              Applicants between this floor and the pass multiplier can be amber
              if they can provide a guarantor.
            </span>
          </label>

          <label className="grid gap-1 text-sm lg:col-span-2">
            <span className="font-medium text-slate-800">
              Reset imported row pointer
            </span>
            <select
              name="resetLastRow"
              defaultValue="false"
        ...
        <div className="grid gap-3 text-sm md:grid-cols-2 xl:grid-cols-5">
          <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
            ...
            <div className="font-medium mt-1">
              {property.advertisedRentMonthly
                ? formatMoney(property.advertisedRentMonthly)
                : "Not set"}
            </div>
          </div>
          <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
            ...
            <div className="font-medium mt-1">
              {property.googleFormImportEnabled ? "Enabled" : "Disabled"}
            </div>
          </div>
          <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
            <div className="text-slate-500">Active tenancy check</div>
            <div className="font-medium mt-1">
              {hasActiveTenancy ? "Automatic import blocked" : "Eligible"}
            </div>
          </div>
          <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
            <div className="text-slate-500">Last checked</div>
            <div className="font-medium mt-1">
              {fmtDateTime(property.googleLastCheckedAt)}
            </div>
          </div>
          <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
            <div className="text-slate-500">Last synced</div>
            <div className="font-medium mt-1">
              {fmtDateTime(property.googleLastImportedAt)}
            </div>
          </div>
          <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
            <div className="text-slate-500">Last imported row</div>
            <div className="font-medium mt-1">
              {property.googleLastImportedRow ?? "—"}
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600 space-y-2">
          <div>
            <strong>Sheet:</strong> {property.googleSheetId ?? "Not set"}
          </div>
          <div>
            <strong>Tab:</strong> {property.googleSheetTabName ?? "Not set"}
          </div>
          <div>
            <strong>Last synced:</strong> {fmtDateTime(property.googleLastImportedAt)}
          </div>
          <div>
            <strong>Latest sync error:</strong> {property.googleSyncError ?? "None"}
          </div>
          <div className="text-xs text-slate-500">
            Vercel cron route: <code>/api/cron/google-form-sync</code>. Set{" "}
            <code>CRON_SECRET</code>, <code>EMAIL_TO</code>, and{" "}
            <code>GOOGLE_SERVICE_ACCOUNT_JSON</code> in your deployment environment.
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className="rounded-xl border bg-white p-4 space-y-2">
          ...
          <div
            className={`text-sm ${statusLabel(property.mortgage?.productEndDate).tone}`}
          >
            {property.mortgage?.productEndDate
              ? `Product ends ${fmt(property.mortgage.productEndDate)}`
              : "Product end not set"}
          </div>
        </div>
        ...
        <div className="rounded-xl border bg-white p-4 space-y-2">
          ...
          <div
            className={`text-sm ${statusLabel(property.insurancePolicy?.renewalDate).tone}`}
          >
            {property.insurancePolicy?.renewalDate
              ? `Renewal ${fmt(property.insurancePolicy.renewalDate)}`
      ...
      <div className="border rounded-xl bg-white p-4">
        <div className="flex justify-between mb-3">
          ...
          <Link
            href={`/tenancies/new?propertyId=${property.id}`}
      ...
      <div className="border rounded-xl bg-white p-4">
        <div className="flex justify-between mb-3">
          ...
          <Link href="/applicants" className="text-blue-600">
            View all
          </Link>
        </div>
        <table className="w-full text-sm">
