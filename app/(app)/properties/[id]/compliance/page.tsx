...
export default async function PropertyCompliancePage({
  ...
  async function saveCompliance(formData: FormData) {
    ...
    const inspectionData = {
      lastDate: String(formData.get("inspectionLastDate") ?? "").trim()
        ? new Date(String(formData.get("inspectionLastDate")))
        : null,
      nextDue: String(formData.get("inspectionNextDue") ?? "").trim()
        ? new Date(String(formData.get("inspectionNextDue")))
        : null,
  ...
  const byType = new Map(
    property.compliance.map((item) => [item.type as ComplianceType, item]),
  );
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <form action={saveCompliance} className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <label className="grid gap-1 text-sm">
                  ...
                  <input
                    name={`${type}_lastDone`}
                    type="date"
                    defaultValue={fmtDate(item?.lastDone)}
                ...
                <label className="grid gap-1 text-sm">
                  ...
                  <input
                    name={`${type}_expiresOn`}
                    type="date"
                    defaultValue={fmtDate(item?.expiresOn)}
                    className="rounded border px-3 py-2"
                  />
                </label>
              </div>
        ...
        <section className="space-y-3 rounded-xl border bg-white p-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="grid gap-1 text-sm">
              ...
              <input
                name="inspectionLastDate"
                type="date"
                defaultValue={fmtDate(inspection?.lastDate)}
                className="rounded border px-3 py-2"
              />
            </label>
            ...
            <label className="grid gap-1 text-sm">
              ...
              <input
                name="inspectionNextDue"
                type="date"
                defaultValue={fmtDate(inspection?.nextDue)}
                className="rounded border px-3 py-2"
              />
            </label>
          </div>
